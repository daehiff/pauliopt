...
def gene_uccsd_oplist(num_orbitals, num_particles, qubit_mapping="bravyi_kitaev"):
    ...
    for i in var_form._hopping_ops:
        ...
        for j in ps:
            t.append(
                pauliString(
                    j["label"], real=j["coeff"]["real"], imag=j["coeff"]["imag"]
                )
            )
        oplist.append(t)
    return oplist
...
def get_qubit_info(atom_config):
    driver = PySCFDriver(
        atom=atom_config, unit=UnitsType.ANGSTROM, charge=0, spin=0, basis="sto3g"
    )
    molecule = driver.run()
...
def get_qubit_op(
    ...
    if remove_list != None:
        ...
        remove_list += [
            x + molecule.num_orbitals - len_freeze_list for x in remove_list
        ]
