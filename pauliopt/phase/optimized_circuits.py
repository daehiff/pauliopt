...
def _validate_loggers(loggers: AnnealingLoggers) -> Tuple[Optional[AnnealingCostLogger],
    ...
    if log_start is not None and not isinstance(log_start, AnnealingCostLogger):
        raise TypeError(
            f"Expected AnnealingCostLogger, found {type(log_start)}")
    if log_iter is not None and not isinstance(log_iter, AnnealingIterLogger):
        raise TypeError(
            f"Expected AnnealingCostLogger, found {type(log_iter)}")
...
class OptimizedPhaseCircuit:
    ...
    def __init__(self, phase_block: Union[PhaseCircuit, PhaseCircuitView],
        ...
        if not isinstance(phase_block, PhaseCircuit):
            raise TypeError(
                f"Expected PhaseCircuit, found {type(phase_block)}.")
    ...
    def anneal(self,
               num_iters: int, *,
               schedule: Union[StandardTempSchedule,
                               TempSchedule] = ("linear", 1.0, 0.1),
        ...
        for layer in reversed(self._cx_block[:layer_idx]):
            ...
            for q in qubits_spanned:
                ...
                if incident_gate is not None:
                    ...
                    new_qubits_spanned.update(
                        {incident_gate[0], incident_gate[1]})
            for incident_gate in incident_gates:
                # will first undo the gate ...
                conj_by.appendleft(incident_gate)
    ...
    def to_svg(self, *,
        ...
        if not isinstance(hscale, (int, float)) or hscale <= 0.0:
            raise TypeError(
                "Keyword argument 'hscale' must be positive float.")
        if not isinstance(vscale, (int, float)) or vscale <= 0.0:
            raise TypeError(
                "Keyword argument 'vscale' must be positive float.")
    ...
    def _to_svg(self, *,
        ...
        pre_cx_gates = [gate for layer in reversed(
            cx_block) for gate in layer.gates]
        ...
        for i, gadget in enumerate(gadgets):
            ...
            if len(gadget.qubits) > 1:
                ...
                builder.text((x + delta_snd + 2 * r, text_y),
                             str(angle), font_size=font_size)
            else:
                for q in gadget.qubits:
                    y = pad_y + (q + 1) * line_height
                    builder.circle((x, y), r, fill)
                builder.text((x + r, y - line_height // 3),
                             str(angle), font_size=font_size)
        ...
        for q in range(num_qubits):
            ...
            _builder.text(
                (0, y), f"{str(q):>{num_digits}}", font_size=font_size)
            _builder.text((width - pad_x + r, y),
                          f"{str(q):>{num_digits}}", font_size=font_size)
        _builder >>= builder
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "You must install the 'IPython' library.")
        return SVG(svg_code)
