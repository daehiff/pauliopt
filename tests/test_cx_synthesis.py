...
class TestCXSynthesis(unittest.TestCase):
    def setUp(self):
        ...
        self.row_matrix = [
            c.parity_matrix(parities_as_columns=False) for c in self.circuit
        ]
            ...
            for col_wise in [True, False]:
                for method in synthesis_methods:
                    with self.subTest(i=i, method=method, parities_as_columns=col_wise):
                        ...
                        synthesized = self.circuit[i].to_qiskit(
        ...
        for i in range(self.n_tests):
            for column_wise in [True, False]:
                for (
                    ...
                    synthesis_methods
                ):  # TODO when more synthesis methods are available, not all are up-to-permutation.
                    with self.subTest(
                        i=i, method=method, parities_as_columns=column_wise
                    ):
                        baseline = self.circuit[i].to_qiskit("naive")
                        permuted = self.circuit[i].to_qiskit(
                            method, True, parities_as_columns=column_wise
                        )
                        unpermute(permuted)
